...
export default function AtendimentoMobileScreen({ token }: { token: string }) {
  ...
  const startNewOrder = () => {
    ...
    if (tipoRetirada === 'delivery' && cliente) {
      // mesmo cliente: pula direto para os itens do próximo pedido
      setCurrentStep(4);
  ...
  type StepKey = 'telefone' | 'cliente' | 'entrega' | 'itens' | 'pagamento' | 'revisao';
  const STEPS: { id: number; key: StepKey; label: string; icon: React.ComponentType<{ size?: number; className?: string }> }[] = [
    ...
    { id: 2, key: 'cliente', label: 'Cliente', icon: User },
    { id: 3, key: 'entrega', label: 'Entrega', icon: MapPin },
    { id: 4, key: 'itens', label: 'Itens', icon: ShoppingBag },
    { id: 5, key: 'pagamento', label: 'Pagamento', icon: CreditCard },
    { id: 6, key: 'revisao', label: 'Revisão', icon: ClipboardCheck },
  ];
  ...
  const isStepDone = (key: StepKey): boolean => {
    switch (key) {
      case 'telefone':
        return isTelefoneOk;
      case 'cliente':
        return Boolean(cliente) || Boolean(clienteNome.trim());
      case 'entrega':
        return isEnderecoOk;
      case 'itens':
  ...
  const stepSummary = (key: StepKey): string => {
    switch (key) {
      ...
      case 'entrega':
        return tipoRetirada === 'retirada' ? 'Retirada no balcão' : enderecoLivre || '—';
      case 'itens':
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      <div className="mx-auto w-full max-w-2xl px-4 py-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
        ...
        <div className="mt-3 space-y-3">
          ...
          {currentStep === 2 && (
            <section className="rounded-2xl border border-[#EA1D2C]/30 bg-zinc-900 p-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-sm font-black text-white">2) Cliente</h2>
                  <p className="text-[11px] text-zinc-500">
                    {cliente ? 'Confirme ou ajuste o nome' : 'Sem cadastro — informe o nome e siga'}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-3">
                <input
                  ...
                  autoComplete="name"
                  autoFocus={!cliente}
                  placeholder={cliente ? 'Nome (opcional, edite se preciso)' : 'Nome do cliente (obrigatório)'}
                  className="w-full rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-3 text-base text-white placeholder:text-zinc-600 outline-none focus:border-[#EA1D2C]/60 focus:ring-2 focus:ring-[#EA1D2C]/20"
                />
              </div>
          ...
          {currentStep === 3 && (
            <section className="rounded-2xl border border-[#EA1D2C]/30 bg-zinc-900 p-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-sm font-black text-white">3) Entrega</h2>
                  <p className="text-[11px] text-zinc-500">Escolha entrega ou retirada no balcão</p>
                </div>
              </div>
          ...
          {currentStep === 5 && (
            <section className="rounded-2xl border border-[#EA1D2C]/30 bg-zinc-900 p-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-sm font-black text-white">5) Pagamento</h2>
          ...
          {/* ETAPA 6 — REVISÃO / SALVAR / IMPRIMIR */}
          {currentStep === 6 && !savedOrderId && (
            <section className="rounded-2xl border border-[#EA1D2C]/30 bg-zinc-900 p-4">
              <div className="flex items-center gap-2">
                <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#EA1D2C]/15 text-[#EA1D2C]">
                ...
                <div>
                  <h2 className="text-sm font-black text-white">6) Revisão</h2>
